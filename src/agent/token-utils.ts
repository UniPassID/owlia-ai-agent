 ...
 */
export function lookupTokenSymbol(address: string, chainId: string): string | null {
  if (!address || !chainId) {
    return null;
  }
 ...
 */
export function lookupTokenDecimals(symbol: string, chainId: string): number | null {
  if (!symbol) {
    return null;
  }
