...
} from '../user/entities/user-deployment.entity';
import { User } from '../user/entities/user.entity';
import { stringify as uuidStringify } from 'uuid';
import { fromBytes, getAddress } from 'viem';
...
export class MonitorService {
  ...
  rpcUrls: Record<NetworkDto, string>;
  ...
  constructor(
    ...
    private userDeploymentRepo: Repository<UserDeployment>,
  ...
  async checkUserPositions(
    ...
    this.rebalanceLogger.startCapture(sessionId, {
      ...
      userAddress: getAddress(fromBytes(deployment.address, 'hex')),
  ...
  async evaluateUserPrecheckByAddress(address: string, network: NetworkDto) {
    ...
    return this.evaluateUserPrecheckByDeployment(deployment as UserDeployment);
  }

  async evaluateUserPrecheckByDeployment(deployment: UserDeployment) {
  ...
  private async executeRebalance(
    ...
    const wallet = getAddress(fromBytes(user.owner, 'hex'));
    ...
    const targetLendingSupplyPositions: any[] = [];
    const targetLiquidityPositions: any[] = [];
    ...
    for (const position of strategy.positions) {
      if (position.type === 'supply') {
        ...
        const lpPosition = this.extractLpPosition(position);
    ...
    if (verification.success && verification.confirmed) {
      ...
      if (
        ...
        (await new JsonRpcProvider(
          this.rpcUrls[getNetworkDto(deployment.chainId)],
        ).getCode(safeAddress)) !== '0x'
      ) {
        deployment.status = UserDeploymentStatus.Deployed;
        await this.userDeploymentRepo.save(deployment);
        this.logger.log(
          `Deployment ${deployment.id} status updated to ${UserDeploymentStatus.Deployed}`,
        );
      }
  ...
  private async saveRebalanceSnapshot(
    ...
    const { ...parsedTransactionWithoutRawLogs } = parsedTransaction;
    const txTime =
  ...
  private extractLpPosition(position: StrategyPosition): any {
