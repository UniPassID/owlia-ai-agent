...
export class RebalanceSummaryService {
  ...
  async generateExecResult(
    ...
    try {
      ...
      this.logger.error(
        `Error generating execResult: ${error.message}`,
        error.stack,
      );
      return null;
    }
    ...
    if (parsed.steps && Array.isArray(parsed.steps)) {
      parsed.steps.forEach((step, idx) => {
        ...
        if (idx === 0) {
          if (contentLen > 35) {
            this.logger.warn(
              `Step[${idx}].content length (${contentLen}) exceeds 35 chars`,
            );
          }
          if (reasonLen > 100) {
            this.logger.warn(
              `Step[${idx}].reason length (${reasonLen}) exceeds 100 chars`,
            );
          }
        } else {
          if (contentLen > 50) {
            this.logger.warn(
              `Step[${idx}].content length (${contentLen}) exceeds 50 chars`,
            );
          }
          if (reasonLen > 300) {
            this.logger.warn(
              `Step[${idx}].reason length (${reasonLen}) exceeds 300 chars`,
            );
          }
        }
      });
