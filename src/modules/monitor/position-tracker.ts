...
export function trackPositionFlows(
  ...
  for (const { txHash, parsed } of parsedTransactions) {
    ...
    const actionsWithContext: Array<{
    ...
    for (let i = 0; i < parsed.actions.length; i++) {
      ...
      if (
        action.protocol === Protocol.UNISWAP_V3 &&
        action.type === RebalanceActionType.POOL_BURN
      ) {
      ...
      if (
        action.protocol === Protocol.UNISWAP_V3 &&
        action.type === RebalanceActionType.POOL_COLLECT
      ) {
      ...
      if (
        action.protocol === Protocol.UNISWAP_V3 &&
        action.type === RebalanceActionType.POOL_MINT
      ) {
  ...
  for (const position of positions.values()) {
    ...
    for (const event of position.events) {
      ...
      if (
        ...
        (event.type === RebalanceActionType.POOL_MINT ||
          event.type === RebalanceActionType.ADD_LIQUIDITY)
      ) {
      ...
      if (
        ...
        if (event.timestamp) {
          if (
            !position.lastCollectTimestamp ||
            event.timestamp > position.lastCollectTimestamp
          ) {
    ...
    if (position.firstMintTimestamp && position.lastCollectTimestamp) {
      position.holdingDurationSeconds =
        position.lastCollectTimestamp - position.firstMintTimestamp;
    }
    ...
    position.formatted = {
      ...
      holdingDuration: position.holdingDurationSeconds
        ? formatDuration(position.holdingDurationSeconds)
        : undefined,
    };
  }
 ...
 */
export function formatPositionTracking(
  ...
  for (const position of summary.positions.values()) {
    ...
    lines.push(
      `Tokens: ${position.token0Symbol || position.token0} / ${position.token1Symbol || position.token1}`,
    );
    ...
    if (position.events.length > 0) {
      ...
      for (const event of position.events) {
        ...
        const amount0Formatted =
          formatAmount(event.token0Amount.toString(), token0Info.decimals) ||
          event.token0Amount.toString();
        const amount1Formatted =
          formatAmount(event.token1Amount.toString(), token1Info.decimals) ||
          event.token1Amount.toString();
    ...
    if (position.formatted?.holdingDuration) {
      ...
      if (position.firstMintTimestamp) {
        lines.push(
          `    From: ${new Date(position.firstMintTimestamp * 1000).toISOString()}`,
        );
      }
      if (position.lastCollectTimestamp) {
        lines.push(
          `    To: ${new Date(position.lastCollectTimestamp * 1000).toISOString()}`,
        );
      }
      lines.push('');
    }
    ...
    lines.push(
      `    ${position.token0Symbol || 'Token0'}: ${position.formatted?.mintToken0}`,
    );
    lines.push(
      `    ${position.token1Symbol || 'Token1'}: ${position.formatted?.mintToken1}`,
    );
    lines.push('');

    lines.push('  📤 Total Withdrawn (Outflows):');
    lines.push(
      `    ${position.token0Symbol || 'Token0'}: ${position.formatted?.withdrawToken0}`,
    );
    lines.push(
      `    ${position.token1Symbol || 'Token1'}: ${position.formatted?.withdrawToken1}`,
    );
    lines.push('');

    lines.push('  💰 Total Fees Collected:');
    lines.push(
      `    ${position.token0Symbol || 'Token0'}: ${position.formatted?.feesToken0}`,
    );
    lines.push(
      `    ${position.token1Symbol || 'Token1'}: ${position.formatted?.feesToken1}`,
    );
    lines.push('');
    ...
    lines.push(
      `    ${position.token0Symbol || 'Token0'}: ${net0}${position.formatted?.netLpToken0Change}`,
    );
    lines.push(
      `    ${position.token1Symbol || 'Token1'}: ${net1}${position.formatted?.netLpToken1Change}`,
    );
    lines.push('-'.repeat(80));
  }
...
export function trackLendingPositions(
  ...
  for (const { txHash, parsed } of parsedTransactions) {
    for (const action of parsed.actions) {
      ...
      if (
        (action.protocol !== Protocol.AAVE &&
          action.protocol !== Protocol.EULER) ||
        (action.type !== RebalanceActionType.SUPPLY &&
          action.type !== RebalanceActionType.WITHDRAW)
      ) {
        continue;
      }
      ...
      if (action.type === RebalanceActionType.SUPPLY) {
        ...
        if (parsed.timestamp) {
          if (
            !position.lastWithdrawTimestamp ||
            parsed.timestamp > position.lastWithdrawTimestamp
          ) {
  ...
  for (const position of positions.values()) {
    ...
    for (const event of position.events) {
      ...
      event.formattedRunningBalance = formatAmount(
    ...
    position.totalInterestEarned =
      position.totalWithdrawn - position.totalSupplied;

    // Calculate holding duration
    if (position.firstSupplyTimestamp && position.lastWithdrawTimestamp) {
      position.holdingDurationSeconds =
        position.lastWithdrawTimestamp - position.firstSupplyTimestamp;
    }
      ...
      if (event.type === RebalanceActionType.SUPPLY) {
          ...
          if (supplyAmount <= remainingWithdraw) {
            ...
            const cycle: LendingCycle = {
              ...
              holdingDurationSeconds:
                supplyEvent.timestamp && event.timestamp
                  ? event.timestamp - supplyEvent.timestamp
                  : undefined,
            };
        ...
        if (remainingWithdraw > 0n) {
          ...
          if (cycles.length > 0) {
            ...
            lastCycle.profit =
              lastCycle.withdrawnAmount - lastCycle.supplyAmount;
          }
        }
        ...
        for (const cycle of cycles) {
          if (
            cycle.profit === 0n &&
            cycle.withdrawnAmount > cycle.supplyAmount
          ) {
    ...
    for (const cycle of position.cycles) {
      cycle.formatted = {
        supplyAmount:
          formatAmount(cycle.supplyAmount.toString(), tokenInfo.decimals) ||
          cycle.supplyAmount.toString(),
        withdrawnAmount:
          formatAmount(cycle.withdrawnAmount.toString(), tokenInfo.decimals) ||
          cycle.withdrawnAmount.toString(),
        profit:
          formatAmount(cycle.profit.toString(), tokenInfo.decimals) ||
          cycle.profit.toString(),
        holdingDuration: cycle.holdingDurationSeconds
          ? formatDuration(cycle.holdingDurationSeconds)
          : undefined,
      };
    }
    ...
    position.formatted = {
      totalSupplied:
        formatAmount(position.totalSupplied.toString(), tokenInfo.decimals) ||
        position.totalSupplied.toString(),
      totalWithdrawn:
        formatAmount(position.totalWithdrawn.toString(), tokenInfo.decimals) ||
        position.totalWithdrawn.toString(),
      totalInterestEarned:
        formatAmount(
 ...
 */
export function formatLendingPositions(
  ...
  for (const position of summary.positions.values()) {
    ...
    if (position.events.length > 0) {
      ...
      for (const event of position.events) {
        ...
        const amountFormatted =
          formatAmount(event.token0Amount.toString(), tokenInfo.decimals) ||
          event.token0Amount.toString();
        ...
        lines.push(`       Tx: ${event.txHash}`);
        lines.push(
          `       Amount: ${event.token0Symbol || 'Token'}: ${amountFormatted}`,
        );

        // Show running balance after this event
        if (event.runningBalance !== undefined) {
          const balanceFormatted =
            event.formattedRunningBalance || event.runningBalance.toString();
          const isNearZero =
            event.runningBalance >= -1000000000000000n &&
            event.runningBalance <= 1000000000000000n; // Within 0.001 for 18 decimals

          if (isNearZero) {
            lines.push(
              `       → Balance after: ${balanceFormatted} ${event.token0Symbol || 'Token'} ⚠️  (Near Zero)`,
            );
          } else {
            lines.push(
              `       → Balance after: ${balanceFormatted} ${event.token0Symbol || 'Token'}`,
            );
          }
        }
      }
    ...
    if (position.formatted?.holdingDuration) {
      ...
      if (position.firstSupplyTimestamp) {
        lines.push(
          `    From: ${new Date(position.firstSupplyTimestamp * 1000).toISOString()}`,
        );
      }
      if (position.lastWithdrawTimestamp) {
        lines.push(
          `    To: ${new Date(position.lastWithdrawTimestamp * 1000).toISOString()}`,
        );
      }
      lines.push('');
    }
    ...
    lines.push(
      `    ${position.tokenSymbol || 'Token'}: ${position.formatted?.totalSupplied}`,
    );
    lines.push('');

    lines.push('  💸 Total Withdrawn:');
    lines.push(
      `    ${position.tokenSymbol || 'Token'}: ${position.formatted?.totalWithdrawn}`,
    );
    lines.push('');
    ...
    lines.push(
      `    ${position.tokenSymbol || 'Token'}: ${interestSign}${position.formatted?.totalInterestEarned}`,
    );
    lines.push('');
    ...
    const tokenInfo = getTokenInfo(position.token, chainId);
    const finalBalanceFormatted =
      formatAmount(finalBalance.toString(), tokenInfo.decimals) ||
      finalBalance.toString();
    const isFinalNearZero =
      finalBalance >= -1000000000000000n && finalBalance <= 1000000000000000n;

    lines.push('  💼 Current Balance (Remaining in Position):');
    if (isFinalNearZero) {
      lines.push(
        `    ${position.tokenSymbol || 'Token'}: ${finalBalanceFormatted} ⚠️  (Near Zero - Position Closed)`,
      );
    } else if (finalBalance > 0n) {
      lines.push(
        `    ${position.tokenSymbol || 'Token'}: ${finalBalanceFormatted} (Still Active)`,
      );
    } else {
      lines.push(
        `    ${position.tokenSymbol || 'Token'}: ${finalBalanceFormatted}`,
      );
    }
    lines.push('');
    ...
    if (position.cycles.length > 0) {
      lines.push(
        `  🔄 Supply-Withdraw Cycles (${position.cycles.length} cycles):`,
      );
      lines.push('');
      ...
      for (let i = 0; i < position.cycles.length; i++) {
        ...
        lines.push(
          `      💵 Supply: ${cycle.formatted?.supplyAmount} ${position.tokenSymbol || 'Token'}`,
        );
        ...
        for (const withdrawEvent of cycle.withdrawEvents) {
          ...
          lines.push(
            `      💸 Withdraw: ${cycle.formatted?.withdrawnAmount} ${position.tokenSymbol || 'Token'}`,
          );
        ...
        lines.push(
          `      ${profitLabel}: ${profitSign}${cycle.formatted?.profit} ${position.tokenSymbol || 'Token'}`,
        );
