...
export class CostCalculatorService {
  ...
  private readonly swapCostSafeAddress = '0x0000000000000000000000000000000000000001';
  ...
  private deductHolding(holdings: Map<string, number>, symbol: string, amount: number): void {
  ...
  private async refreshSwapCostData(chainId: string, force: boolean = false): Promise<void> {
    if (!chainId || this.refreshingSwapChains.has(chainId)) {
      return;
    }

    const cacheEntry = this.swapCostCache.get(chainId);
    const isStale = !cacheEntry || Date.now() - cacheEntry.updatedAt > this.swapCostTtlMs;
    if (!force && !isStale) {
      return;
    }
    ...
    if (stableTokens.length < 2) {
      this.warnOnce(this.missingStableTokenChains, chainId,
    ...
    try {
      ...
      this.logger.log(`calculate_swap_cost_batch payload: ${JSON.stringify(payload)}`)

      const response = await this.agentService.callMcpTool<CalculateRebalanceCostBatchResponse>(
        'calculate_swap_cost_batch',
        payload,
      );

      const results = this.normalizeDictionaryResponse<CalculateRebalanceCostResult>(response);
      ...
      if (Object.keys(matrix).length === 0) {
        this.logger.warn(`Swap cost refresh for chain ${chainId} returned no usable entries`);
        return;
      }
  ...
  private buildSwapCostRequests(chainId: string, stableTokens: StableTokenInfo[]): SwapCostRequestPayload[] {
    const network = this.getNetworkName(chainId);
    if (!network) {
      this.warnOnce(this.missingNetworkChains, chainId,
    ...
    for (const from of stableTokens) {
      for (const to of stableTokens) {
        ...
        const amountIn = this.usdToTokenUnits(this.swapCostBaseAmountUsd, from.decimals);
        const amountOut = this.usdToTokenUnits(this.swapCostBaseAmountUsd, to.decimals);
  ...
  private getDefaultLendingProtocol(chainId: string): 'aave' | 'euler' | 'venus' {
    if (chainId === '56') {
      return 'venus';
    }
    return 'aave';
  ...
  private getStableTokensForChain(chainId: string): StableTokenInfo[] {
    ...
    return Object.entries(chainTokens)
      ...
      .map(([symbol, address]) => ({
        ...
        address,
        decimals: typeof chainDecimals[symbol] === 'number' ? chainDecimals[symbol] : 18,
      }));
  }

  private getStableSymbolsForChain(chainId: string): string[] {
    return this.getStableTokensForChain(chainId).map(token => token.symbol.toUpperCase());
  }

  public createStableHoldingsMap(chainId: string, holdings: Record<string, number>): Map<string, number> {
  ...
  public async estimateSwapCostForToken(
    ...
    const dynamicCost = await this.getDynamicSwapCost(chainId, sourceSymbol, normalizedTarget, amountUsd);
  ...
  private normalizeHoldings(holdings: Record<string, number>): Map<string, number> {
    const map = new Map<string, number>();
    if (!holdings) {
      return map;
    }
  ...
  private async getDynamicSwapCost(
    ...
    if(scaledCost < 0) {
      this.logger.warn(`scaledCost is negative ${scaledCost}, will return 0 instead`)
      return 0;
    }
  ...
  getTargetTokensForOpportunity(
    ...
    if (opp.type === 'supply') {
      ...
      const lpInfo = this.findLpPositionInfo(opp.poolAddress!, lpSimulations, dexPools);
      if (!lpInfo) return [];
      ...
      const token0Symbol = this.getTokenSymbolFromAddress(lpInfo.token0Address, chainId, dexPools);
      const token1Symbol = this.getTokenSymbolFromAddress(lpInfo.token1Address, chainId, dexPools);
   ...
   */
  private getTokenSymbolFromAddress(address: string, chainId: string, dexPools: Record<string, any>): string {
