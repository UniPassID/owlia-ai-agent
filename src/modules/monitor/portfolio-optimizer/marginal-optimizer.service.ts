...
export class MarginalOptimizerService {
  ...
  async optimizePortfolio(
    ...
    while (
      remainingCapital >= options.incrementSize &&
      iteration < maxIterations
    ) {
      iteration++;

      // Dynamic increment: if remaining < 2x incrementSize, use all remaining; otherwise use incrementSize
      const baseIncrementAmount =
        remainingCapital < options.incrementSize * 2
          ? remainingCapital
  ...
  private async calculateMarginalNetAPY(
    ...
    const effectiveHoldingPeriodDays =
      opp.type === 'supply' ? 7 : holdingPeriodDays;
    ...
    if (useRealCost) {
      ...
      for (const [symbol, total] of Object.entries(
        holdingsState.currentHoldings,
      )) {
      ...
      const availableStableHoldings =
        this.costCalculator.createStableHoldingsMap(chainId, availableHoldings);
      ...
      for (const { symbol, amount: requiredAmount } of targetTokens) {
        ...
        if (deficit > 0) {
          ...
          const isFirstTimeSwapForToken =
            !holdingsState.swappedTokens.has(symbol);
      ...
      swapCost = this.estimateSwapCostFallback(
        incrementAmount,
        hasExistingPosition,
      );
    }
  ...
  private updateUsedHoldingsAndMarkSwapped(
    ...
    for (const { symbol, amount: requiredAmount } of targetTokens) {
      ...
      holdingsState.usedHoldings[symbol] =
        (holdingsState.usedHoldings[symbol] || 0) + requiredAmount;
      ...
      const alreadyUsed =
        (holdingsState.usedHoldings[symbol] || 0) - requiredAmount; // Before this allocation
      const availableAmount = totalHoldings - alreadyUsed;
      const deficit = requiredAmount - availableAmount;
