...
export function convertPlanToSteps(
  ...
  const currentPositions: CurrentPosition[] = Array.isArray(
    plan.currentPositions,
  )
    ? plan.currentPositions
    : [];
  const currentValue = currentPositions.reduce(
    (sum, pos) => sum + toNumber(pos.value),
    0,
  );
  const currentAPY =
    currentValue > 0
      ? currentPositions.reduce(
          (sum, pos) => sum + toNumber(pos.apy) * toNumber(pos.value),
          0,
        ) / currentValue
      : 0;
  ...
  const opportunities: RebalanceOpportunity[] = Array.isArray(
    plan.opportunities,
  )
    ? plan.opportunities
    : [];
  ...
  if (opportunities.length > 0) {
    ...
    steps.push({
      ...
      status: 'success',
      metadata:
        isNoRebalanceNeeded && recommendation
  ...
  if (opportunities.length > 0) {
    ...
    opportunities.forEach((opp) => {
      const value = toNumber(
        opp.amount ?? opp.targetAmount0 ?? opp.targetAmount1,
      );
      const apy = toNumber(opp.expectedAPY);
    ...
    const costEstimate =
      Array.isArray(plan.costEstimates) && plan.costEstimates.length > 0
  ...
  const isNoRebalanceCompleted =
    jobStatus === 'completed' && opportunities.length === 0;
...
function generateSummary(
  ...
  const opportunities = Array.isArray(plan.opportunities)
    ? plan.opportunities
    : [];

  if (jobStatus === 'completed' && opportunities.length === 0) {
    const apyText =
      currentAPY && currentAPY > 0 ? ` at ${currentAPY.toFixed(2)}% APY` : '';
  ...
  if (jobStatus === 'completed' && opportunities.length > 0) {
    ...
    const gasEstimate = resolveGasEstimate(
      Array.isArray(plan.costEstimates) ? plan.costEstimates[0] : undefined,
    );
  ...
  if (jobStatus === 'failed') {
    const errorMessage =
      execResult?.error || execResult?.errorMessage || 'Unknown error';
...
function formatProtocolName(protocol: string): string {
  ...
  return (
    nameMap[protocol] || protocol.charAt(0).toUpperCase() + protocol.slice(1)
  );
}
...
function resolveGasEstimate(costEstimate?: RebalanceCostEstimate): number {
  ...
  if (
    typeof costEstimate.netGasUsd === 'number' &&
    costEstimate.netGasUsd > 0
  ) {
    return costEstimate.netGasUsd;
  }
  if (
    typeof costEstimate.gasEstimate === 'number' &&
    costEstimate.gasEstimate > 0
  ) {
