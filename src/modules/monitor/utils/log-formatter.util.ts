  ...
  if (session.metadata) {
    if (session.metadata.precheckResult) {
      ...
      if (precheck.portfolioApy !== undefined) {
        lines.push(
          `Portfolio APY:          ${precheck.portfolioApy.toFixed(2)}%`,
        );
      }
      ...
      if (precheck.differenceBps !== undefined) {
        lines.push(
          `Difference:             ${precheck.differenceBps.toFixed(2)} bps`,
        );
      }
      if (precheck.totalPortfolioValueUsd !== undefined) {
        lines.push(
          `Total Portfolio Value:  $${formatCurrency(precheck.totalPortfolioValueUsd)}`,
        );
      }
      if (precheck.gasEstimate !== undefined) {
        lines.push(
          `Gas Estimate:           $${formatCurrency(precheck.gasEstimate)}`,
        );
      }
      if (precheck.breakEvenTimeHours !== undefined) {
        lines.push(
          `Break-even Time:        ${precheck.breakEvenTimeHours.toFixed(2)} hours`,
        );
      }
      if (precheck.netGainUsd !== undefined) {
        lines.push(
          `Net Gain (Annual):      $${formatCurrency(precheck.netGainUsd)}`,
        );
      }
      if (precheck.bestStrategy) {
        lines.push(
          `Selected Strategy:      ${precheck.bestStrategy.name || 'Unknown'}`,
        );
      }
      lines.push('');
    }
