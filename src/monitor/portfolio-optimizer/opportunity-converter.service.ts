  ...
  convertToOpportunities(
    ...
    for (const data of supplyData) {
      ...
      for (const opp of data.opportunities) {
        ...
        const apyFunctions = this.apyCalculator.createSupplyAPYFunctions(opp, totalCapital, chainId);
    ...
    for (const sim of lpSimulations) {
      ...
      const token0Address = this.extractTokenAddress(poolAddress, dexPools, 'token0');
      const token1Address = this.extractTokenAddress(poolAddress, dexPools, 'token1');
      const targetTokens = [token0Address, token1Address].filter(t => t);
      const apyFunctions = this.apyCalculator.createLpAPYFunctions(sim, totalCapital, chainId);
  ...
  private extractTokenAddress(
    ...
    for (const [poolAddr, poolData] of Object.entries(dexPools)) {
      if (poolAddr.toLowerCase() === normalizedPoolAddress) {
        ...
        return currentSnapshot[`${tokenKey}Address`] || currentSnapshot[tokenKey] || '';
      }
    }
    return '';
  }
