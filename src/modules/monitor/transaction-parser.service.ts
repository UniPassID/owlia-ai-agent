...
export class TransactionParserService {
  ...
  private readonly EVENT_SIGNATURES: EventSignature[] = [
    ...
    },
    {
      name: 'Borrow',
      signature:
        'Borrow(address indexed reserve,address user,address indexed onBehalfOf,uint256 amount,uint256 borrowRateMode,uint256 borrowRate,uint16 indexed referralCode)',
      protocol: Protocol.AAVE,
  ...
  private readonly poolTokenCache = new Map<
    string,
  ...
  async parseTransaction(
    ...
    try {
      ...
      this.logger.log(
        `Parsed ${actions.length} rebalance actions from transaction`,
      );
      ...
      this.logger.error(
        `Error parsing transaction ${txHash}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async parseLog(
    ...
    this.logger.debug(
      `Parsing ${eventSig.name} event from ${eventSig.protocol}`,
    );

    try {
      const action = await this.parseEventByType(
        log,
        eventSig,
        eventIndex,
        provider,
      ...
      this.logger.warn(
        `Failed to parse ${eventSig.name} event: ${error.message}`,
      );
      return null;
    }
  }
  ...
  private async parseEventByType(
    ...
    switch (eventSig.actionType) {
      ...
      case RebalanceActionType.POOL_COLLECT: {
        const result = await this.parseLiquidityEvent(
          log,
          decoded,
          eventSig,
          provider,
    ...
    while (i < actions.length) {
      ...
      if (
        ...
        if (nextAction.type === RebalanceActionType.REMOVE_LIQUIDITY) {
          ...
          const collectAction =
            i + 2 < actions.length &&
            actions[i + 2].type === RebalanceActionType.POOL_COLLECT
              ? actions[i + 2]
              : null;
          ...
          if (collectAction) {
            ...
            collectTokens.forEach((collectToken, idx) => {
              ...
              const collectAmount = parseFloat(
                collectToken.amountFormatted || '0',
              );
              ...
              liquidityParts.push(
                `${this.formatTokenAmount(burnToken?.amountFormatted || '0')} ${symbol}`,
              );

              // Fee amount
              if (feeAmount > 0.000001) {
                feeParts.push(
                  `${this.formatTokenAmount(feeAmount.toString())} ${symbol}`,
                );
              } else {
              ...
              totalParts.push(
                `${this.formatTokenAmount(collectToken.amountFormatted || '0')} ${symbol}`,
              );
            });
            ...
            const poolName =
              poolSymbols.length >= 2
                ? `${poolSymbols[0]}/${poolSymbols[1]}`
                : poolSymbols.join('/');

            // Add tick range info if available
            const tickInfo =
              action.tickLower !== undefined && action.tickUpper !== undefined
            ...
            const burnTokens = action.tokens
              .map(
                (t) =>
                  `${this.formatTokenAmount(t.amountFormatted)} ${t.symbol || 'TOKEN'}`,
              )
              .join(' + ');
            this.logger.log(
  ...
  private describeAction(action: RebalanceAction): string {
    ...
    switch (action.type) {
      ...
      case RebalanceActionType.SUPPLY: {
        ...
        const vault = action.contractAddress
          ? ` (vault: ${action.contractAddress.slice(0, 10)}...)`
          : '';
  ...
  private async parseLiquidityEvent(
    ...
    if (eventSig.protocol === Protocol.UNISWAP_V3) {
      ...
      if (
        eventSig.name === 'IncreaseLiquidity' ||
        eventSig.name === 'DecreaseLiquidity'
      ) {
        ...
        const amount0 = BigInt(
          amount0Raw !== undefined ? amount0Raw.toString() : '0',
        );
        const amount1 = BigInt(
          amount1Raw !== undefined ? amount1Raw.toString() : '0',
        );
        ...
        const tickLowerRaw =
          (decoded.args as any)?.tickLower ?? decoded.args?.[2];
        const tickUpperRaw =
          (decoded.args as any)?.tickUpper ?? decoded.args?.[3];
        const amount0Raw = (decoded.args as any)?.amount0 ?? decoded.args?.[5];
        const amount1Raw = (decoded.args as any)?.amount1 ?? decoded.args?.[6];

        tickLower =
          tickLowerRaw !== undefined ? Number(tickLowerRaw) : undefined;
        tickUpper =
          tickUpperRaw !== undefined ? Number(tickUpperRaw) : undefined;
        ...
        const resolvedTokens = await this.buildPoolTokenAmounts(
          log.address,
          amount0,
          amount1,
          provider,
        ...
        // args array includes all params in order
        const tickLowerRaw =
          (decoded.args as any)?.tickLower ?? decoded.args?.[1];
        const tickUpperRaw =
          (decoded.args as any)?.tickUpper ?? decoded.args?.[2];
  ...
  private async resolveTokensFromNearbyPoolEvents(
    ...
    const poolEventSig = this.EVENT_SIGNATURES.find(
      (sig) =>
        sig.protocol === Protocol.UNISWAP_V3 && sig.name === poolEventName,
    );
    if (!poolEventSig?.abi) {
      return null;
    }
    ...
    for (const idx of indices) {
      ...
      const resolved = await this.buildPoolTokenAmounts(
        candidate.address,
        amount0,
        amount1,
        provider,
  ...
  private async parseSwapEvent(
    ...
    if (
      eventSig.protocol === Protocol.OKX_ROUTER &&
      eventSig.name === 'OrderRecord'
    ) {
    ...
    if (
      eventSig.protocol === Protocol.KYBERSWAP_ROUTER &&
      eventSig.name === 'Swapped'
    ) {
  ...
  private async parseLendingEvent(
    ...
    if (eventSig.protocol === Protocol.AAVE) {
      ...
      if (
        eventSig.actionType === RebalanceActionType.SUPPLY ||
        eventSig.actionType === RebalanceActionType.BORROW
      ) {
      ...
      if (amountRaw === undefined && Array.isArray(args)) {
        const candidateIndex =
          args.length >= 2 ? args.length - 2 : args.length - 1;
        if (candidateIndex >= 0) {
      ...
      const amountMeta = venusAmountField[eventSig.name] ?? {
        field: '',
        index: 1,
      };
      const amountRaw =
        (amountMeta.field ? decoded.args?.[amountMeta.field] : undefined) ??
        decoded.args?.[amountMeta.index];
  ...
  formatParsedTransaction(parsed: ParsedTransaction): string {
    ...
    parsed.actions.forEach((action, idx) => {
      ...
      action.tokens.forEach((token, tokenIdx) => {
        ...
        lines.push(
          `  Token ${tokenIdx + 1}: ${amountDisplay} ${symbolDisplay}`,
        );
