...
export class RebalanceLoggerService {
  ...
  stopCapture(jobId: string, status?: string): void {
    ...
    this.logger.log(
      `Stopped log capture for job ${jobId} with status ${status}`,
    );
  }
  ...
  async saveToFile(
    ...
    try {
      ...
      if (this.config.format === 'text' || this.config.format === 'both') {
        const textFileName = generateLogFileName(
      ...
      if (this.config.format === 'json' || this.config.format === 'both') {
        const jsonFileName = generateLogFileName(
          session.deploymentId,
          jobId,
          'json',
        );
      ...
      this.logger.error(
        `Failed to save log for job ${jobId}: ${(error as Error).message}`,
      );
  ...
  async readLogFile(filePath: string): Promise<string | null> {
    try {
      ...
      this.logger.error(
        `Failed to read log file ${filePath}: ${(error as Error).message}`,
      );
      return null;
    }
  }
  ...
  async cleanupOldLogs(): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to cleanup old logs: ${(error as Error).message}`,
      );
    }
  }
  ...
  startInterception(sessionId: string): void {
    if (this.isIntercepting) {
      this.logger.warn(
        `Already intercepting logs for session ${this.activeInterceptionSession}`,
      );
      return;
    }
  ...
  stopInterception(): void {
    ...
    this.logger.log(
      `Stopped global log interception for session ${this.activeInterceptionSession}`,
    );
